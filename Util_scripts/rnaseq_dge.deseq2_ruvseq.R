...
if (!is.null(args$ruv_colData)){
    ruv_colData <- read.csv(args$ruv_colData, sep='\t')
    new_colData <- merge(x = colData, y = ruv_colData, by = "row.names", all.x = TRUE)
    new_colData <- subset(new_colData, select = -c(rep.y, condition.y) )
    new_colData[is.na(new_colData)] <- 0
    rownames(new_colData) <- new_colData$Row.names
    new_colData[,1] <- NULL
    print(new_colData)
